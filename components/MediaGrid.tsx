...
import { Play, Lock, Clock, ExternalLink } from 'lucide-react';
...
const MediaGrid: React.FC<MediaGridProps> = ({ items, onItemClick, showPremiumBadge = false }) => {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-2 sm:gap-4 md:gap-6">
      {items.map((item) => {
        const isExternalVideo = item.mediaType === 'video';
        const className = "group relative bg-black rounded-xl sm:rounded-2xl overflow-hidden border border-zinc-900 hover:border-yellow-500/50 transition-all duration-300 hover:shadow-2xl hover:shadow-yellow-500/10 cursor-pointer flex flex-col h-full";
        ...
        const cardContent = (
          <>
            <div className="relative aspect-video overflow-hidden">
              ...
              {/* External video tag */}
              {isExternalVideo && (
                <div className="absolute top-1 left-1 sm:top-3 sm:left-3 bg-yellow-500 text-black px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md text-[9px] sm:text-xs font-bold uppercase tracking-wider flex items-center shadow-md">
              ...
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 transform scale-75 group-hover:scale-100">
                 <div className="w-10 h-10 sm:w-14 sm:h-14 bg-yellow-500 rounded-full flex items-center justify-center shadow-lg shadow-yellow-500/40">
                   {isExternalVideo ? (
                     <ExternalLink className="w-4 h-4 sm:w-6 sm:h-6 text-black" />
                   ) : (
            ...
            <div className="p-2 sm:p-4 flex flex-col flex-1">
              <div className="flex justify-between items-start mb-1 sm:mb-2">
                <h3 className="font-semibold text-zinc-200 text-xs sm:text-base line-clamp-2 sm:line-clamp-1 group-hover:text-yellow-400 transition-colors leading-tight flex items-center gap-1">
                  {item.title}
                  {isExternalVideo && (
                    <ExternalLink className="w-3.5 h-3.5 text-yellow-500 flex-shrink-0 inline" />
                  )}
                </h3>
              </div>
        ...
        if (isExternalVideo) {
          const url = item.externalUrl || item.redirectUrl || item.sourceUrl || '#';
          return (
            <a 
              key={item.id}
              href={url}
